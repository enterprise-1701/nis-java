...
public class ProductCatalogTests extends NextLinkBase {
	...
	public void getProductCatalog(ITestContext context, Hashtable<String, String> data) throws Throwable {
		...
		try {
			...
			restActions.assertTrue( "Successful".equals(responseObj.getHdr().getResult()), "Hdr Result IS NOT 'Successful' BUT IT SHOULD BE" );
	        ...
	        clientResponse = restActions.postClientResponse(surl, productCatalogRequestBody, headers, urlQueryParameters,
					RESTConstants.APPLICATION_JSON);
			...
			restActions.assertTrue( "CONTINUE".equals(responseObj.getResult().getResponseCode()), "Response Code IS NOT 'CONTINUE' BUT IT SHOULD BE" );
			...
			do {
				...
				clientResponse = restActions.postClientResponse(surl, productCatalogRequestBody,
						headers, urlQueryParameters, RESTConstants.APPLICATION_JSON);
			...
			if( "OK".equals(responseObj.getResult().getResponseCode()) ) {
				...
				for ( WSStoredValue stored : catResponse.getCatalog().getStoredValue() ) {
					restActions.assertTrue(null != stored.getName(),
						"STORED VALUE NAME IS NULL BUT IT SHOULD NOT BE");
				...
				for ( WSProduct product : catResponse.getCatalog().getProduct() ) {
					...
					restActions.assertTrue(null != product.getName(),
						"PRODUCT NAME IS NULL BUT IT SHOULD NOT BE");
			...
			restActions.assertTrue("OK".equals(responseObj.getResult().getResponseCode()), "Response Code IN NOT 'OK' BUT IT SHOULD BE");
			restActions.assertTrue("Successful".equals(responseObj.getHdr().getResult()), "Result IS NOT 'Successful' BUT IT SHOULD BE");
