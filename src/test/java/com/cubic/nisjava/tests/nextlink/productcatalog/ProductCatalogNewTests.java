...
public class ProductCatalogNewTests extends RESTEngine {
	...
	public void getNewProductCatalog(ITestContext context, Hashtable<String, String> data) throws Throwable {
		...
		try {
			...
			ClientResponse clientResponse = restActions.getClientResponse(newProdCatURL, headers, urlQueryParameters, RESTConstants.APPLICATION_JSON);
			...
			for ( WSStoredValue storedValue : storedValueList ) {
				restActions.assertTrue(storedValue.getName() != null, 
					"STORED VALUE IS NULL BUT IT SHOULD NOT BE" );
				
				restActions.assertTrue(storedValue.getAmount() != null, 
						"AMOUNT IS NULL BUT IT SHOULD NOT BE" );
				
				restActions.assertTrue(storedValue.getAmount() > 0, 
						"AMOUNT IS =< ZERO BUT IT SHOULD NOT BE" );	
				
				List<String> loadTypeList = storedValue.getLoadType();	
				if ( null != loadTypeList ) {
					restActions.assertTrue( loadTypeList.size() == 0,
						"LOAD TYPE LIST HAS SIZE > 0 BUT IT SHOULD NOT" );
				}
			}
			...
			for ( WSProduct product : catResponse.getCatalog().getProduct() ) {
				...
				restActions.assertTrue(product.getName() != null, 
						"NAME IS NULL BUT IT SHOULD NOT BE" );
				
				restActions.assertTrue(product.getName().length() > 0, 
						"NAME IS NULL BUT IT SHOULD NOT BE" );
				
				restActions.assertTrue(product.getNameShort() != null, 
						"NAME SHORT IS NULL BUT IT SHOULD NOT BE" );
				
				restActions.assertTrue(product.getNameShort().length() > 0, 
						"NAME SHORT IS NULL BUT IT SHOULD NOT BE" );
				...
				restActions.assertTrue(product.getPrice() != null, 
						"PRICE IS NULL BUT IT SHOULD NOT BE" );
				
				restActions.assertTrue(product.getPrice() > 0, 
						"PRICE IS =< ZERO BUT IT SHOULD NOT BE" );
			}
		}
		finally {
